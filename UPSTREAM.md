...
| `6308afb` | `[Patches] RestoreBrightnessText`: the collection replaces MGS1's Option → SCREEN help texture with a four-line version that drops the line naming the back button, and the remaining lines render 12 rows low with an opaque band over the background above them. Filters just the four patches that do it, so the paragraph reads as the game wrote it, in its own place — unlike `DisableCDROM`, which fixes it only by turning off every patch and the achievements with them | every MGS1 player in the collection who wants achievements on | **verified in game 2026-09-03** with `DisableRAM`/`DisableCDROM` both `false`, i.e. every collection patch active and achievements live: MGS1 (USA)'s brightness screen shows all six lines, at the higher position, with no band, and the log names the four patches filtered. Other versions still need their own four names |
