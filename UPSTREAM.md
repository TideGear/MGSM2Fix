...
independent of the Integral port. They are in `src/ketchup.{cpp,h}` and belong
...
* **`ApplyBlock` warns when a record is not fully mirrored.** A write that runs
  ...
  used to be silent while the log said the patch loaded — it once cost a patch
...
* **`ApplyPPF3` validates the whole record chain before applying any of it.** A
  ...
  applying garbage in a loop — a 306 MB log, once. A patch is now either wholly
