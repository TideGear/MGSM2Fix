...
| `6308afb` | `[Patches] RestoreBrightnessText`: the collection replaces MGS1's Option → SCREEN help texture with a four-line version that drops the line naming the back button, and the remaining lines render 12 rows low with an opaque band over the background above them. Filters just the four patches that do it, so the paragraph reads as the game wrote it, in its own place — unlike `DisableCDROM`, which fixes it only by turning off every patch and the achievements with them | every MGS1 player in the collection who wants achievements on | built 2026-09-03; **needs a run with `DisableCDROM = false`** to prove the targeted filter does the work (with the flag on, the master toggle already hides the bug) |
