...
Copy the mods folder into your MGS1 game folder. It holds three sets:
  mods/INTEGRAL/INTEGRAL/0   Integral disc 1 - nine INTEGRAL_disc1_en_*.ppf
  mods/INTEGRAL/INTEGRAL/1   Integral disc 2 - nine INTEGRAL_disc2_en_*.ppf
...
*_unlock_* file. Ketchup loads every PPF in a folder, in name order, and
INTEGRAL_vr_en_movie.ppf is built on top of INTEGRAL_vr_en_missions.ppf: install
the seven VR files together, never the movie one alone.
...
disc-swap prompts, which only the story's disc change can reach; disc 2 is
...
Uninstall: remove the nine INTEGRAL_discN_en_*.ppf files from each numbered
